...
import Variables from "@/uibuilder/variables/index.vue";
    ...
    <div ref="uipmodal" class="uip-flex uip-flex-column uip-row-gap-m uip-scale-in uip-w-500 uip-padding-m">
      ...
      <template v-else>
        ...
        <div class="uip-flex uip-flex-middle">
          <div class="uip-flex uip-flex-row uip-flex-between uip-flex-center uip-w-250">
            <template v-for="step in steps">
              <div
                class="uip-border-circle uip-w-22 uip-ratio-1-1 uip-background-muted uip-flex uip-flex-center uip-flex-middle uip-transition-all uip-link-default"
                :class="{ 'uip-text-inverse uip-background-primary': currentStep == step.key || currentStep > step.key }"
                @click="currentStep = step.key"
              >
                <span>{{ step.key }}</span>
              </div>

              <div v-if="step.key < steps.length" class="uip-flex-grow uip-background-grey uip-transition-all" :class="{ 'uip-background-primary': currentStep > step.key }" style="height: 1px"></div>
            </template>
          </div>
        </div>
        <!-- End step display -->
        ...
        <div v-if="currentStep == 1" class="uip-grid-col-1-3 uip-padding-remove-top uip-fade-in">
          ...
          <inline-image-select
            :value="setupDetails.darkLogo"
            :returnData="
              function (d) {
                setupDetails.darkMode = d;
              }
            "
            :args="{ hasPositioning: false }"
          ></inline-image-select>
                ...
                <div class="uip-flex uip-flex-column uip-row-gap-xs uip-flex-left">
                  ...
                  <div class="uip-flex uip-flex-row uip-gap-xs">
                    <template v-for="(item, index) in theme.images">
                      <div
                        class="uip-w-32 uip-ratio-1-1 uip-border-round uip-background-muted uip-background-cover uip-border uip-border-box"
                        @click="theme.activeIndex = index"
                        :style="'background-image: url(' + item + ')'"
                        :class="{ 'uip-border-primary': returnActiveIndex(theme) == index }"
                      ></div>
                    </template>
                  </div>
        ...
        <div v-if="currentStep == 4" class="uip-grid-col-1-3 uip-padding-remove-top uip-fade-in">
          ...
          <user-role-select
            :selected="setupDetails.excludes"
            :placeHolder="strings.selectUsersAndRoles"
            :searchPlaceHolder="strings.searchUsersAndRoles"
            :single="false"
            :updateSelected="
              function (data) {
                setupDetails.excludes = data;
              }
            "
          ></user-role-select>
        </div>
        ...
        <div v-if="currentStep == 5" class="uip-grid-col-1-3 uip-padding-remove-top uip-fade-in">
          ...
          <switch-select
            :value="setupDetails.enableLoginTheme"
            :returnData="
              function (d) {
                setupDetails.enableLoginTheme = d;
              }
            "
            :args="{ asText: true }"
          ></switch-select>
          ...
          <inline-image-select
            :value="setupDetails.loginLogo"
            :returnData="
              function (d) {
                setupDetails.loginLogo = d;
              }
            "
            :args="{ hasPositioning: false }"
          ></inline-image-select>
          ...
          <inline-image-select
            :value="setupDetails.loginBackground"
            :returnData="
              function (d) {
                setupDetails.loginBackground = d;
              }
            "
            :args="{ hasPositioning: false }"
          ></inline-image-select>
        ...
        <div class="uip-flex uip-gap-xs">
          <button class="uip-button-default" @click.stop.prevent="currentStep -= 1" :disabled="currentStep <= 1">{{ strings.previous }}</button>

...
import colorSelect from "@/utility/colorpicker.js";
  ...
  <div class="uip-flex uip-flex-column uip-row-gap-xs uip-position-relative">
    <div class="uip-flex uip-flex-between uip-flex-center">
      <component is="style" scoped>
        .searchfade-enter-active, .searchfade-leave-active { transition: all 0.2s ease; } .searchfade-enter-from, .searchfade-leave-to { opacity: 0; transform: translateX(30px); }
      </component>
      ...
      <template v-for="(style, index) in returnUserStyles">
        <div
          @contextmenu.prevent.stop="
            contextIndex = style.name;
            $refs.colorcontext.show($event);
          "
        ...
        >
          <div class="uip-w-16 uip-border-circle uip-ratio-1-1 uip-background-checkered">
            <div class="uip-w-16 uip-border-circle uip-ratio-1-1 uip-border" :style="{ background: returnStyleBackground(style) }"></div>
          </div>
    ...
    <div class="uip-fade-in uip-position-absolute uip-bottom-0 uip-w-100p" v-if="!scrollBottom && search == ''" style="pointer-events: none">
      <div
        style="position: absolute; top: 0px; background-image: linear-gradient(to top, var(--uip-color-base-0), transparent); transform: translateY(-100%); height: 60px; width: 100%; z-index: 9"
      ></div>
    </div>
    ...
    <contextmenu ref="colorcontext" :disableTeleport="true">
      <div class="uip-padding-xs uip-flex uip-flex-column uip-text-weight-normal uip-text-s">
        ...
        <a
          class="uip-link-danger uip-flex uip-gap-m uip-flex-between uip-flex-center hover:uip-background-muted uip-border-round uip-padding-xxs"
          @click.prevent="
            delete this.uipApp.data.themeStyles[contextIndex];
            $refs.colorcontext.close();
          "

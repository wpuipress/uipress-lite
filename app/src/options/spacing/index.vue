  ...
  <div class="uip-flex uip-flex-column uip-row-gap-xs">
    ...
    <div class="uip-grid-col-1-3">
      <div class="uip-text-muted uip-flex uip-flex-center uip-text-s uip-flex-start uip-padding-top-xxs">
        <span>{{ strings.padding }}</span>
      </div>

      <div class="uip-flex uip-flex-column uip-row-gap-xs">
        <toggle-switch
          :options="spacingOptions"
          :activeValue="spacing.padding.preset"
          :returnValue="
            function (data) {
              spacing.padding.preset = data;
            }
          "
        ></toggle-switch>
        ...
        <div class="uip-flex uip-gap-xs uip-flex-center" v-if="spacing.padding.preset == 'custom'">
          <value-units
            :value="{ value: spacing.padding.left, units: spacing.padding.units }"
            :returnData="
              (data) => {
                handleUnitChange(spacing.padding, data);
              }
            "
            class="uip-flex-grow"
          />
          <toggle-switch
            :options="syncOptions"
            :activeValue="spacing.padding.sync"
            :returnValue="
              (data) => {
                spacing.padding.sync = data;
              }
            "
            style="width: 50%"
          ></toggle-switch>
      ...
      <div class="uip-flex uip-flex-column uip-row-gap-xs">
        <toggle-switch
          :options="spacingOptions"
          :activeValue="spacing.margin.preset"
          :returnValue="
            function (data) {
              spacing.margin.preset = data;
            }
          "
        ></toggle-switch>
        ...
        <div class="uip-flex uip-gap-xs uip-flex-center" v-if="spacing.margin.preset == 'custom'">
          <value-units
            :value="{ value: spacing.margin.left, units: spacing.margin.units }"
            :returnData="
              (data) => {
                handleUnitChange(spacing.margin, data);
              }
            "
            class="uip-flex-grow"
          />
          <toggle-switch
            :options="syncOptions"
            :activeValue="spacing.margin.sync"
            :returnValue="
              (data) => {
                spacing.margin.sync = data;
              }
            "
            style="width: 50%"
          ></toggle-switch>

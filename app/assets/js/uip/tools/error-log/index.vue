        ...
        <dropdown pos="bottom left">
          ...
          <template v-slot:content>
            <div class="uip-padding-s uip-flex uip-flex-column uip-row-gap-xxs uip-w-200">
              ...
              <div
                class="uip-flex uip-flex-between uip-gap-l hover:uip-background-muted uip-padding-xxs uip-border-rounder uip-flex-center uip-cursor-pointer"
                :class="{ 'uip-background-grey': order == 'desc' }"
                @click="order = 'desc'"
              >
              ...
              <div
                class="uip-flex uip-flex-between uip-gap-l hover:uip-background-muted uip-padding-xxs uip-border-rounder uip-flex-center uip-cursor-pointer"
                :class="{ 'uip-background-grey': order == 'asc' }"
                @click="order = 'asc'"
              >
              ...
              <template v-if="err.stackTrace && err.stackTrace.length > 0">
                ...
                <div class="uip-flex uip-flex-column uip-padding-left-s">
                  <template v-for="(item, index) in err.stackTrace">
                    <div
                      class="uip-text-xs uip-text-muted uip-padding-xxs uip-border-left uip-padding-left-s uip-circle-before uip-position-relative"
                      :class="[{ 'uip-padding-remove-top': index == 0 }, { 'uip-padding-remove-bottom uip-border-transparent': index == err.stackTrace.length - 1 }]"
                    >
                      {{ item }}
                    </div>
                  </template>

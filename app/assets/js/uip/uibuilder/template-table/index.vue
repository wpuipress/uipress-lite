  ...
  <div class="uip-flex uip-flex-column uip-h-100p uip-w-100p uip-background-muted" style="min-height: 100dvh">
    ...
    <div class="uip-flex uip-flex-column uip-flex-center uip-padding-l">
      ...
      <div class="uip-flex uip-flex-column uip-row-gap-m uip-w-1000 uip-max-w-100p">
        ...
        <component is="style">
          tr:hover { box-shadow: var(--uip-box-shadow); } tr:hover td:first-child{ border-radius:8px 0 0 8px; } tr:hover td:last-child{ border-radius:0 8px 8px 0; } .uip-template-table tr:first-child
          td:first-child {border-radius: 8px 0 0 0;} .uip-template-table tr:first-child td:last-child {border-radius: 0 8px 0 0;} .uip-template-table tr:last-child td:first-child {border-radius: 0 0 0
          8px;} .uip-template-table tr:last-child td:last-child {border-radius: 0 0 8px 0;} .uip-template-table tr:last-child td {border: none !important;}
        </component>
        ...
        <!-- Table -->
        <TransitionGroup name="list" tag="table" class="uip-background-transparent uip-template-table" style="border-collapse: collapse; border-spacing: 0">
          <template v-for="(template, index) in returnTableData" :key="template.id">
            <tr class="uip-link-default uip-border-rounder" @click="loadTemplate(template.id)" @contextmenu.prevent.stop="$refs['templatemenu-' + index][0].show($event)">
              <!-- Template Title -->
              <td class="uip-border-bottom uip-padding-s uip-background-default">
                <div class="uip-flex uip-flex-column uip-gap-xxs">
                  <div class="uip-flex uip-gap-xs uip-flex-center">
                    ...
                    <div
                      class="uip-w-9 uip-ratio-1-1 uip-border-circle uip-opacity-70"
                      :class="template.status == 'publish' ? 'uip-background-green' : 'uip-background-orange'"
                      style="returnActiveIndicatorStyle(template.status)"
                    ></div>
                  </div>
              ...
              <td class="uip-border-bottom uip-padding-s uip-max-w-100 uip-background-default">
                <switch-select
                  :args="{ asText: true, small: true, options: activeSwitchOptions }"
                  @click.stop
        ...
        <template v-for="(template, index) in returnTableData" :key="template.id">
          <contextmenu :ref="'templatemenu-' + index">
            <div class="uip-padding-xs uip-flex uip-flex-column uip-text-weight-normal uip-text-s">
              ...
              <a
                class="uip-link-muted uip-flex uip-flex-center uip-flex-between uip-gap-m uip-padding-xxs hover:uip-background-muted uip-border-rounder"
                @click.prevent="
                  duplicateTemplate(template.id);
                  $refs['templatemenu-' + index][0].close();
                "
              >
              ...
              <a
                class="uip-link-danger uip-flex uip-flex-center uip-flex-between uip-gap-m uip-padding-xxs hover:uip-background-muted uip-border-rounder"
                @click.prevent="

  ...
  <div class="uip-flex uip-flex-column uip-row-gap-s">
    ...
    <div v-if="ui.tabs.color" class="uip-flex uip-flex-column uip-padding-left-xs">
      <template v-for="(item, index) in returnColorVars">
        <div class="uip-flex uip-gap-xs uip-flex-center uip-gap-xxxs">
          ...
          <dropdown :ref="'coloreditor-' + index" pos="left center" :snapX="['#uip-block-settings', '#uip-template-settings', '#uip-global-settings']" class="uip-flex-grow">
            <template #trigger>
              <div class="uip-flex uip-gap-s uip-flex-center uip-padding-xxs uip-padding-left-xs uip-link-default uip-border-rounder hover:uip-background-muted">
                <div class="uip-w-16 uip-border-circle uip-ratio-1-1 uip-background-checkered">
                  <div class="uip-w-16 uip-border-circle uip-ratio-1-1 uip-border" :style="{ background: returnStyleBackground(item) }"></div>
                </div>
    ...
    <div v-if="ui.tabs.units" class="uip-flex uip-flex-column uip-padding-left-s uip-row-gap-xxs">
      <template v-for="(item, index) in returnUnitsVars">
        <div class="uip-flex uip-gap-xs uip-flex-center uip-gap-xxxs">
          ...
          <dropdown :ref="'uniteditor-' + index" pos="left center" :snapX="['#uip-block-settings', '#uip-template-settings', '#uip-global-settings']" class="uip-flex-grow">
            <template #trigger>
              <div class="uip-flex uip-gap-s uip-flex-center uip-padding-xxs uip-padding-left-xs uip-link-default uip-border-rounder hover:uip-background-muted">
            ...
            <template #content>
              <div class="uip-padding-s uip-flex uip-flex-column uip-row-gap-xs uip-w-240">
                ...
                <div class="uip-grid-col-1-3 uip-padding-left-xs">
                  <div class="uip-text-muted uip-flex uip-flex-center uip-text-s uip-no-wrap">
                    <span>{{ ui.strings.styleName }}</span>
                  </div>
                  <input
                    :disabled="!item.user"
                    @input="item.name = cleanKeyName(item.name)"
                    class="uip-input uip-w-100p"
                    type="text"
                    v-model="item.name"
                    :placeholder="ui.strings.styleName"
                    autofocus
                  />

                  <div class="uip-text-muted uip-flex uip-flex-center uip-text-s uip-no-wrap">
                    <span>{{ ui.strings.value }}</span>
                  </div>
                  <value-units
                    :value="splitUnitVal(item)"
                    size="xsmall"
                    :returnData="
                      (data) => {
                        setUnitVal(data, item);
    ...
    <div v-if="ui.tabs.font" class="uip-flex uip-flex-column uip-padding-left-s uip-row-gap-xxs">
      <template v-for="(item, index) in returnFontsVars">
        <div class="uip-flex uip-gap-xs uip-flex-center uip-gap-xxxs">
          <!--Units dropdown dropdown-->
          <dropdown :ref="'fonteditor-' + index" pos="left center" :snapX="['#uip-block-settings', '#uip-template-settings', '#uip-global-settings']" class="uip-flex-grow">
            <template #trigger>
              <div class="uip-flex uip-gap-s uip-flex-center uip-padding-xxs uip-padding-left-xs uip-link-default uip-border-rounder hover:uip-background-muted">
    ...
    <!-- New color dropdown-->
    <contextmenu ref="newcolourdrop" :snapRight="true" :snapX="['#uip-block-settings', '#uip-template-settings', '#uip-global-settings']" :offsetX="-8">
    ...
    <!-- New Unit dropdown-->
    <contextmenu ref="newunitdrop" :snapRight="true" :snapX="['#uip-block-settings', '#uip-template-settings', '#uip-global-settings']" :offsetX="-8">
      <div class="uip-padding-s uip-flex uip-flex-column uip-row-gap-s uip-w-280">
        ...
        <div class="uip-grid-col-1-3 uip-padding-left-xs">
          ...
          <input
            ...
            class="uip-input uip-flex-grow uip-w-100p"
            type="text"
            v-model="newUnitItem.name"
            :placeholder="ui.strings.styleName"
            autofocus
          />
          ...
          <value-units
            :value="splitUnitVal(newUnitItem)"
            size="xsmall"
            :returnData="
              (data) => {
                setUnitVal(data, newUnitItem);
              }
            "
            class="uip-w-100p"
          ></value-units>
    ...
    <!-- New Text dropdown-->
    <contextmenu ref="newtextdrop" :snapRight="true" :snapX="['#uip-block-settings', '#uip-template-settings', '#uip-global-settings']" :offsetX="-8">
      <div class="uip-padding-s uip-flex uip-flex-column uip-row-gap-s uip-w-280">
        <div class="uip-flex uip-flex-between uip-flex-center">
        ...
        <div class="uip-grid-col-1-3 uip-padding-left-xs">
          ...
          <input @input="newUnitItem.name = cleanKeyName(newUnitItem.name)" class="uip-input uip-w-100p" type="text" v-model="newUnitItem.name" :placeholder="ui.strings.styleName" autofocus />

...
import FontFamilies from "@/js/uip/data/fonts.js";
  ...
  <div class="uip-flex uip-flex-column uip-row-gap-s">
    ...
    <div class="uip-grid-col-1-3">
      ...
      <template v-if="option.font == 'custom'">
        ...
        <div class="uip-text-muted uip-flex uip-flex-center uip-text-s">
          <span>{{ strings.customFontName }}</span>
        </div>
      ...
      <div class="uip-flex uip-flex-column uip-row-gap-xs uip-flex-start">
        <toggle-switch
          ...
          :activeValue="option.size.preset"
          :returnValue="
            function (data) {
              option.size.preset = data;
            }
          "
        ></toggle-switch>
      </div>
      ...
      <!--Color -->
      <div class="uip-text-muted uip-flex uip-flex-center uip-text-s">
        <span>{{ strings.colour }}</span>
      </div>

      <div class="uip-flex uip-gap-xs uip-flex-no-wrap uip-flex-center">
        <dropdown pos="left center" :snapX="['#uip-block-settings', '#uip-template-settings', '#uip-global-settings']" ref="colorDrop" class="uip-w-100p">
          <template #trigger>
            <colorBox
              :backgroundStyle="returnTextColor"
              :text="option.color.value"
              :remove="
                () => {
                  option.color = {};
                }
      ...
      <div class="uip-text-muted uip-flex uip-flex-center uip-text-s">
        <span>{{ strings.align }}</span>
      </div>

      <div class="uip-flex uip-flex-column uip-row-gap-xs uip-flex-start">
        <toggle-switch
          :options="alignOptions"
          :activeValue="option.align"
          :returnValue="
            function (data) {
              option.align = data;
            }
      ...
      <div class="uip-text-muted uip-flex uip-flex-center uip-text-s">
        <span>{{ strings.lineHeight }}</span>
      </div>
      <value-units
        :value="option.lineHeight"
        :returnData="
          function (data) {
            option.lineHeight = data;
          }
        "
      />
      ...
      <div class="uip-text-muted uip-flex uip-flex-center uip-text-s">
        <span>{{ strings.fontWeight }}</span>
      </div>

      <div class="uip-flex uip-row-gap-xs">
        <default-select
          :value="returnOption.weight"
          :args="{ options: fontWeights }"
          :returnData="
            function (e) {
              option.weight = e;
            }
      ...
      <div class="uip-text-muted uip-flex uip-flex-center uip-text-s">
        <span>{{ strings.textTransform }}</span>
      </div>

      <div class="uip-flex uip-row-gap-xs">
        <default-select
          :value="returnOption.transform"
          :args="{ options: textTransform }"
          :returnData="
            function (e) {
              option.transform = e;
            }
      ...
      <div class="uip-text-muted uip-flex uip-flex-center uip-text-s">
        <span>{{ strings.decoration }}</span>
      </div>

      <div class="uip-flex uip-row-gap-xs">
        <toggle-switch
          :options="decorationOptions"
          :activeValue="option.decoration"
          :returnValue="
            function (data) {
              option.decoration = data;
            }
          "
        ></toggle-switch>
      </div>

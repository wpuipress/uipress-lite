  ...
  <div class="uip-flex uip-flex-column uip-row-gap-xs uip-padding-left-xs">
    ...
    <div class="uip-grid-col-1-3">
      <div class="uip-text-muted uip-flex uip-flex-center uip-text-s">
        <span>{{ strings.shadowColour }}</span>
      </div>

      <colorBox
        @click="$emit('request-screen', returnFillScreen)"
        :backgroundStyle="returnShadowColor"
        :text="returnShadow.color.value"
        :remove="
          () => {
            shadowOptions.color = {};
          }
    ...
    <div class="uip-grid-col-1-3">
      ...
      <div class="uip-flex uip-gap-xs uip-flex-no-wrap uip-flex-center">
        <toggle-switch
          ...
          :activeValue="shadowOptions.position"
          :returnValue="
            function (data) {
              shadowOptions.position = data;
            }
          "
        ></toggle-switch>
      </div>

...
<template>
  <div class="uip-flex uip-flex-column uip-row-gap-xs uip-w-100p">
    ...
    <div v-if="option.action == 'phpFunction'" class="uip-grid-col-1-3">
      <div class="uip-text-muted uip-text-s uip-flex uip-flex-center">
        <span>{{ strings.functionName }}</span>
      </div>
    ...
    <div v-if="option.action == 'userMeta'" class="uip-grid-col-1-3">
      ...
      <template v-if="option.objectOrSingle == 'object'">
        ...
        <div class="uip-margin-bottom-xs">
          <uip-input
            :value="option.userMetaObjectKey"
            :returnData="
              function (e) {
                option.userMetaObjectKey = e;
              }
            "
            :args="{ metaKey: true }"
          ></uip-input>
    ...
    <div v-if="option.action == 'siteOption'" class="uip-grid-col-1-3">
      ...
      <div class="">
        <uip-input
          :value="option.siteOptionName"
          :returnData="
            function (e) {
              option.siteOptionName = e;
            }
          "
          :args="{ metaKey: true }"
        ></uip-input>
    ...
    <template v-if="option.action == 'email'">
      <div class="uip-grid-col-1-3">
        <div class="uip-text-muted uip-margin-bottom-xx uip-text-s uip-flex uip-flex-center">
          <span>{{ strings.recipientEmail }}</span>
        </div>
        <div>
    ...
    <div class="uip-grid-col-1-3">
      ...
      <div class="">
        <link-select
          ...
          :returnData="
            function (e) {
              option.redirectURL = e;
            }
          "
          :args="{ hideLinkType: true }"
        ></link-select>
      </div>
    </div>
  </div>

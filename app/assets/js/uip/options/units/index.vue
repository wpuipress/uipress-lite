    ...
    <template v-slot:content>
      <div class="uip-flex uip-flex-column uip-flex-left uip-text-s uip-padding-s">
        <template v-for="item in options">
          <div
            :title="item.label"
            class="uip-link-muted uip-flex uip-flex-center uip-gap-xxs uip-padding-xxxs uip-padding-left-xxs hover:uip-background-muted uip-border-rounder"
            @click="selectItem(item)"
            :class="{ 'uip-text-emphasis uip-background-muted': option == item.value }"
          >
            <span class="uip-w-40">{{ item.value }}</span>
            <span class="uip-text-muted uip-w-100 uip-overflow-hidden uip-no-wrap uip-text-ellipsis" :class="option == item.value ? 'uip-text-emphasis' : 'uip-text-muted'">{{ item.label }}</span>
          </div>
        </template>
      </div>
    </template>

...
import colourStyleEditor from "@/js/uip/components/color-style-editor/index.vue";
import imageBGSelect from "@/js/uip/components/image-select/index.vue";

import backgroundFill from "@/js/uip/options/background-fill/index.vue";
import borderDesigner from "@/js/uip/options/border-designer/index.vue";
import shadowDesigner from "@/js/uip/options/shadow-designer/index.vue";
import outlineDesigner from "@/js/uip/options/outline-designer/index.vue";
...
<template>
  <div class="uip-flex uip-flex-column uip-row-gap-xs">
        ...
        <dropdown pos="left center" :snapX="['#uip-block-settings', '#uip-template-settings', '#uip-global-settings']" ref="backgroundDrop" class="uip-w-100p">
          <template #trigger>
            <colorBox
              ...
              :text="styles.backgroundImage.url"
              :remove="
                () => {
    ...
    <div class="uip-grid-col-1-3">
      <div class="uip-text-muted uip-flex uip-flex-center uip-text-s">
        <span>{{ strings.radius }}</span>
      </div>

      <div class="uip-flex uip-flex-column uip-row-gap-xs">
        <div class="uip-flex uip-gap-xs uip-flex-center">
          <value-units
            :value="{ value: styles.radius.topleft, units: styles.radius.units }"
            :returnData="
              (data) => {
                handleUnitChange(styles.radius, data);
              }
            "
            class="uip-flex-grow"
          />
          <toggle-switch
            :options="syncOptions"
            :activeValue="styles.radius.sync"
            :returnValue="
              (data) => {
                styles.radius.sync = data;
              }
            "
            style="width: 50%"
          />
          ...
          <div class="uip-flex uip-gap-xs uip-row-gap-xs uip-flex-center">
            <dropdown pos="left center" class="uip-w-100p" :ref="'borderDrop-' + index" :snapX="['#uip-block-settings', '#uip-template-settings', '#uip-global-settings']">
              <template v-slot:trigger>
                <colorBox
                  :backgroundStyle="returnBorderColor(border, index)"
                  :text="border.style"
                  :remove="
                    () => {
                      styles.borders.splice(index, 1);
                    }
                  "
                />
              </template>
              <template v-slot:content>
                <div class="uip-padding-s uip-w-240">
                  <screenControl :startScreen="borderScreen(border, index)" :homeScreen="borderScreen(border, index).component" :closer="$refs['borderDrop-' + index][0].close" :showNavigation="true">
                    <template #componenthandler="{ processScreen, currentScreen, goBack }">
                      <KeepAlive>
                        <component
                          @tab-change="
        ...
        <template v-for="(shadow, index) in styles.shadows">
          <div class="uip-flex uip-gap-xs uip-row-gap-xs uip-flex-center">
            <dropdown pos="left center" class="uip-w-100p" :ref="'shadowDrop-' + index" :snapX="['#uip-block-settings', '#uip-template-settings', '#uip-global-settings']">
              <template v-slot:trigger>
                <colorBox
                  :backgroundStyle="returnShadowColor(shadow)"
                  :text="shadow.position"
                  :remove="
                    () => {
                      styles.shadows.splice(index, 1);
                    }
                  "
                />
              </template>
              <template v-slot:content>
                <div class="uip-padding-s uip-w-240">
                  <screenControl :startScreen="shadowScreen(shadow, index)" :homeScreen="shadowScreen(shadow, index).component" :closer="$refs['shadowDrop-' + index][0].close" :showNavigation="true">
                    <template #componenthandler="{ processScreen, currentScreen, goBack }">
                      <KeepAlive>
                        <component
                          @tab-change="
    ...
    <div class="uip-grid-col-1-3">
      ...
      <div class="uip-flex uip-gap-xs uip-flex-no-wrap uip-flex-center">
        <dropdown pos="left center" :snapX="['#uip-block-settings', '#uip-template-settings', '#uip-global-settings']" ref="outlineDrop" class="uip-w-100p">
          <template #trigger>
            <colorBox
              :backgroundStyle="returnOutlineStyle"
              :text="styles.outline.style"
              :remove="
                () => {
                  styles.outline = {};
                }

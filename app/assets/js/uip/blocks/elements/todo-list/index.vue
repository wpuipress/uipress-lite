...
import { getUserPreference } from "@/js/uip/utility/functions.js";
export default {
  props: {
    display: String,
    name: String,
...
<template>
  <div class="uip-flex uip-flex-column uip-row-gap-s uip-flex-no-wrap">
    <component is="style" scoped> .list-enter-active, .list-leave-active { transition: all 0.5s ease; } .list-enter-from, .list-leave-to { opacity: 0; transform: translateX(30px); } </component>
    ...
    <div class="uip-flex uip-flex-row uip-gap-s uip-padding-xxs uip-tabs">
      <template v-for="tab in tabs">
        <div
          class="uip-link-muted uip-padding-bottom-xs uip-tab"
          :class="{ 'uip-border-bottom-primary uip-text-bold uip-text-emphasis uip-tab-active': activeTab == tab.name }"
        ...
        <div v-for="(item, index) in returnToDos" class="uip-flex uip-flex-row uip-gap-xs" :key="item" :data-index="index">
          ...
          <div class="uip-flex uip-flex-column uip-row-gap-xxxs uip-flex-grow">
            ...
            <textarea
              ...
              class="uip-input uip-text-s uip-text-muted uip-no-resize uip-blank-input uip-list-item-description uip-min-h-20 uip-overflow-hidden"

...
import { renderKeyShortCut } from "@/js/uip/utility/functions.js";
export default {
  props: {
    display: String,
    name: String,
  ...
  <dropdown ref="dropdown" :pos="returnDropPosition" :shortCut="getShortcutValue" :disableTeleport="true" :hover="returnOpenHover">
    <template #trigger>
      <button class="uip-button uip-button-default uip-flex uip-gap-xxs uip-flex-center uip-drop-trigger" :class="returnClasses">
        ...
        <div
          v-if="getShortcut"
          class="uip-flex uip-flex-row uip-padding-left-xxxs uip-padding-right-xxxs uip-border uip-border-round uip-text-s uip-flex-row uip-inline-flex uip-flex-center"
          v-html="getShortcut"
        ></div>
      </button>
    </template>

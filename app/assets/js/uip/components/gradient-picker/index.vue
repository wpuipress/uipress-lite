...
import { parseGradient } from "@/js/uip/utility/gradient-parser.js";
  ...
  <div class="uip-flex uip-flex-column uip-row-gap-s">
    ...
    <div class="" @mouseleave="controlhover = false" @mousemove="setNewControlPos">
      ...
      <svg
        ...
        @click="pushNewColorAtLocation($event)"
        class="uip-h-8 uip-border-rounder uip-overflow-visible uip-background-checkered uip-w-100p"
        :style="'background:' + returnGradAsLinear"
      >
        <!--Controls-->
        <g>
          <!--New point marker-->
          <ellipse v-if="controlhover" :cx="controlNewx" cy="50%" rx="2" ry="2" fill="#fff" stroke="#fff" stroke-width="2" style="filter: drop-shadow(0px 0px 1px grey)" />

          <template v-for="(color, index) in gradient.colorStops">
            <ellipse
              @mousedown.prevent.stop="startPointDrag(color, $event)"
              @contextmenu.prevent.stop="$refs['stopcontext' + index][0].show($event)"
              :cx="color.length.value + '%'"
              cy="50%"
              rx="6"
              ry="6"
              :fill="color.value"
    ...
    <svg
      ref="gradientanglecontrol"
      xmlns="http://www.w3.org/2000/svg"
      @mouseenter="gradientHover = true"
      @mouseleave="gradientHover = false"
      class="uip-border-rounder uip-w-100p uip-h-130 uip-background-checkered"
      :style="'background:' + returnGradAsString"
      viewBox="0 0 240 130"
    >
      <g v-show="gradient.type == 'linear-gradient' && (gradientHover || dragging)">
        ...
        <ellipse ref="anglecenter" cx="50%" cy="50%" rx="4" ry="4" fill="transparent" stroke="#fff" stroke-width="2" style="filter: drop-shadow(0px 0px 1px grey)" />

              ...
              <div class="uip-min-w-200 uip-padding-s uip-flex uip-row-gap-s uip-flex-column">
                ...
                <div class="uip-flex uip-flex-wrap uip-flex-start uip-gap-xs uip-row-gap-xs uip-padding-bottom-remove" style="font-size:14px;max-width:290px;min-width:290px">
                  <template v-for="option in iconsPaged">
                   <span class="uip-icon uip-icon-medium uip-text-xl hover:uip-background-muted uip-padding-xxxs uip-border-round hover:uip-background-grey uip-cursor-pointer uip-flex-no-grow uip-max-w-30 uip-overflow-hidden uip-link-default" @click="chooseIcon(option)">
                     {{option}}
                   </span>
                  </template>
                </div>
                
                <div class="uip-flex uip-flex-between uip-flex-between">
                
                  <button class="uip-button-default uip-border-rounder uip-padding-xxs uip-link-muted" @click="icon.value = ''">
                    {{strings.clearIcon}}
                  </button>
